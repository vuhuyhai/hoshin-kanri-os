...
export default function SwotGuidePage() {
  ...
  return (
    <div className="w-full min-h-full bg-[#F7F5F2]">
      <div className="px-6 lg:px-8 pt-6 lg:pt-8">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          <span className="inline-flex items-center font-display font-bold text-[11px] uppercase tracking-wider px-2.5 py-1 border-2 border-ink bg-white" style={{ boxShadow: '2px 2px 0px #2C2B2B' }}>6 bước · ~15 phút</span>
        </div>
      </div>
      {/* Mobile progress */}
      <div className="lg:hidden flex gap-1.5 px-6 pb-4 overflow-x-auto">
        {GUIDE_STEPS.map((_, i) => (
          <button key={i} onClick={() => scrollTo(i)} className={`w-8 h-8 shrink-0 flex items-center justify-center font-display font-black text-sm border-2 border-black transition-colors ${active === i ? 'bg-[#c73937] text-white' : 'bg-white text-ink'}`}>{i + 1}</button>
        ))}
      </div>
      <div className="flex px-6 lg:px-8 pb-8 gap-8">
        ...
        <div className="flex-1 space-y-6 max-w-3xl">
          ...
          <div className="bg-white border-2 p-6 text-center" style={{ borderColor: '#2C2B2B', boxShadow: '4px 4px 0px #2C2B2B' }}>
            <Link href="/dashboard/discovery/swot">
              <button className="font-display font-black text-sm uppercase tracking-wider px-8 py-3 text-white border-2 border-black transition-all hover:translate-y-[-2px]" style={{ backgroundColor: '#c73937', boxShadow: '4px 4px 0px #2C2B2B' }}>Bắt đầu phân tích SWOT →</button>
            </Link>
