...
const CARD_VARIANTS = {
  default: 'bg-card border-2 border-[var(--color-ink)]',
  accent:  'bg-card border-2 border-[var(--color-ink)] border-l-4 border-l-[var(--color-accent-brand)]',
  warning: 'bg-accent-yellow border-2 border-kpi-attention-strong',
  success: 'bg-accent-lime border-2 border-kpi-healthy-strong',
} as const
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row gap-6">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 text-sm font-semibold uppercase tracking-wide border-2 border-[var(--color-ink)] cursor-pointer transition-colors text-left font-display ${
                  isActive
                    ...
                    : 'bg-card hover:bg-[var(--color-bg-muted-warm)]'
                }`}
              >
        ...
        <select
          ...
          className="w-full border-2 border-[var(--color-ink)] px-3 py-2 text-sm font-semibold uppercase bg-card focus:outline-none mb-4 md:hidden font-display"
        >
          {visibleSections.map(s => (
