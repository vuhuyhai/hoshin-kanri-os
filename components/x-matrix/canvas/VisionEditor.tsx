...
import { useCanvas, useCanEdit } from './state/CanvasContext'
...
export function VisionEditor() {
  ...
  const hasVision = state.data.vision.trim() !== ''
  ...
  return (
    <section className="w-full border-b-[3px] border-ink bg-[var(--bg)] px-4 py-4 lg:px-8">
      ...
      {canEdit ? (
        ...
        <p
          className={`w-full rounded-[var(--radius-md)] border-2 border-ink bg-[var(--bg)] px-4 py-3 font-body text-base ${
            hasVision
              ? 'text-ink'
              : 'italic text-[var(--text-3)]'
          }`}
        >
          {hasVision ? state.data.vision : 'Chưa có tầm nhìn'}
        </p>
      )}
      {canEdit && tooShort && (
