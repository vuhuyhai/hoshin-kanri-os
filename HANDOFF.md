...
- **Latest milestone**: M-Design-Dark-1 (Dark mode foundation full ship — brand + saturated + Pastel cascade + toggle UI 3-option + chart cache bridge, 7 commits `bbdca2d` plan → `5ed573a` refactor → `22cd969` brand foundation → `6ba6e03` saturated + border → `293e6b0` Pastel mute → `d29fc03` toggle UI + cache bridge → `<C7-HASH>` close-out, ~5h45min cumulative work). Trigger: M-Design-3a/3b/Tokens-Cleanup-1 foundation shipped (KPI 8 + Score 4 + Pastel 6 + brand tokens + composite borders) NHƯNG `.dark` block chỉ override shadcn neutral + chart + sidebar (~30 tokens). Toàn bộ NB v3.2 custom (~30+ tokens) thiếu dark variants — brand foundation gap CATASTROPHIC (`--ink #1A1A1A` trên `--bg #1A1A1A` = text invisible). Path γ FULL locked Q1 (all tokens + toggle UI fix 1 milestone) + Q2 invert lab + Q4 β `--brand #E84947` align shadcn `--primary` + Q5 α dark mute pastel + Q6 γ refactor LITERAL→REFERENCE + Q7 α useEffect listen `useTheme().resolvedTheme` + Q8 α 7 commits no split. Total milestone delta ~+386 net LOC within Q1 γ FULL ~6-8h budget.
  ...
  - **Tasks shipped (7 commits)**:
    ...
    7. Commit 7 close-out `<C7-HASH>` (Smoke 8 cases Phase A + HANDOFF §16/§17/§18 + plan doc Section 5/6 archive)
...
- ~~**M-Design-Dark-1**~~ ✅ SHIPPED 2026-05-11 (7 commits `bbdca2d`→`<C7-HASH>`, 5-layer cascade architecture). See "Shipped milestones (recent)" above + §16 + §17 architecture decision 2026-05-11.
