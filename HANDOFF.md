...
## 16. Current State Snapshot (2026-05-11 — post M-Design-Tangential-Cleanup-1)
...
- **Last verified**: 2026-05-11 — post M-Design-Tangential-Cleanup-1 (14 commits cleanup raw Tailwind palette → NB v3.2 tokens, ~200 migrations / ~70 keep-raw / 9 buckets shipped). HEAD `<close-out hash>` (close-out chain post-push). Touch ~65 files across 9 consumer buckets + 2 foundation CSS commits. Cumulative milestone metrics: 14 atomic sub-bucket commits (Foundation `.card-subtle` `b0edc91` + Foundation `.input-brutal` `0d6a792` + lien-he `0d220cc` + Task 1.5 re-audit `a922ed3` + X-Matrix canvas `947a2c4` + Auth `b6846a6` + X-Ray doc-only `955036a` + Annual-review `ce17b7d` + Admin `29fe6f5` + Dashboard 11a/11b/11c `3a3a883`/`cb4a365`/`0825f2c` + SWOT 9a/9b/9c `c8a8e01`/`276307c`/`4a8de48` + close-out). 65% migration ratio cumulative (200 ÷ 304 baseline) — keep-raw justified majority cases via per-occurrence semantic audit (Q2-sub-α dark surface + Q2 extended saturated-brand + Ambig 1 C elevated cards on warm cream + Hoshin Explorer 2-color category data-driven + AI sparkle decorative hex). 0 new tokens (L48 verified existing `--card/--bg-warm/--bg-muted/--bg-paper/--ink/--text-2/--text-3/--brand/--accent-*/--kpi-*` consumes). 0 business logic / state / JSX structure delta. Phase A user-facing smoke deferred manual anh Vũ Hải khi tiện cross 10 routes (HIGH: lien-he + Auth × 4 + Dashboard top-level + KPI tracker; MEDIUM: Settings + Discovery hub + X-Matrix canvas + SWOT workshop/canvas/synthesis; LOW: admin Hoshin Explorer). Previous: M-Design-Dark-1.5 (Soft hide dark mode UI hotfix, 1 commit `67919e0`, ~30 phút work, 2 NEW pattern lessons L56 + L57). Previous: M-Design-Dark-1 (Dark mode foundation full ship → hidden UI post-1.5, 7 commits `bbdca2d`→`74b1c65`, 5-layer cascade architecture).

  - **Files changed (cumulative)**: ~65 .tsx files MODIFIED across 9 consumer buckets + `app/globals.css` 2 foundation CSS edits (`.card-subtle` line 809-814 `--white` → `--card` + `.input-brutal` parallel) + `plans/_archive/shipped-2026-05-11/M-Design-Tangential-Cleanup-1-plan.md` (NEW archived) + `plans/README.md` (entry update) + `HANDOFF.md` (this close-out chain).

  - **Bucket ratio table** (real migration ÷ baseline grep count):

    | Bucket | Files | Real | Keep-raw | Ratio | Commit |
    |---|---:|---:|---:|---:|---|
    | lien-he | 1 | 5 | 0 | 100% | `0d220cc` |
    | X-Matrix canvas | 3 | 1 | 4 | 20% | `947a2c4` |
    | Auth | 4 | 6 | 4 | 67% | `b6846a6` |
    | X-Ray (doc-only) | 3 | 0 | 10 | 0% | `955036a` |
    | Annual-review | 5 | 14 | 3 | 82% | `ce17b7d` |
    | Admin | 7 | 2 | 18 | 10% | `29fe6f5` |
    | Dashboard 11a/11b/11c | 14 | 47 | 19 | 71% | `3a3a883`/`cb4a365`/`0825f2c` |
    | SWOT 9a/9b/9c | 32 | 130 | 47 | 74% | `c8a8e01`/`276307c`/`4a8de48` |
    | **TOTAL** | **~65** | **~200** | **~70** | **~65%** | — |

  - **3 architectural changes**:
    1. **Q1 β commit cadence directory-bucket atomic proven 14 commits + 9 sub-buckets scalable** (Pattern L50 reinforced lần 6). Dashboard 11a/11b/11c sub-split (surface vs KPI status vs discovery decoration) + SWOT 9a/9b/9c sub-split (Workshop+Chat vs Context+Drafts+Canvas vs TOWS+Synthesis+Finalize) scale gracefully cho large diff bucket (>200 LOC). Reusable cho future ≥5-sub-bucket cleanup milestones.
    2. **Q2 default mapping table + Q2-sub-α extended cross 5 bucket precedent (Layout/Page footer + Auth dark panel + X-Ray CTA banner + Hoshin Explorer 2-color category + AI sparkle decorative hex)**: Keep-raw scope established uniform cross consumer buckets. White-on-saturated-surface intentional contrast (Q2-sub-α + Q2 extended saturated-brand) + Ambig 1 C elevated `bg-white` cards on warm cream + data-driven hex from framework definition keep-raw justified majority via per-occurrence semantic audit. Pattern L59 NEW emerged.
    3. **Q6 β atomic per concern proven L48 + L50 cumulative**: 2 foundation commits (.card-subtle + .input-brutal) ship FIRST trước 9 consumer buckets. `:root` `--card` token reuse validated (existing shadcn token, NO net-new introduction). Tailwind v4 `@theme inline --color-card` mirror exists pre-milestone — L48 foundation parity preserved.

  - **7 Q-decisions locked (cumulative cross Task 1 + Task 1.5 + Task 8 + SWOT pre-decision lock)**:
    - Q1 β Commit cadence directory-bucket atomic (14 commits proven)
    ...
    - Q2 extended Keep-raw white text on saturated-brand surface (Layout header button + Auth Google btn + X-Ray pills/checkmark + Annual-review status pills)
    - Q2 Ambig 1 C Keep-raw elevated `bg-white` cards on warm cream (Auth Google btn + X-Ray option pill + SWOT framework card)
    - Q4 α Skip shadcn primitives backdrop alpha (dialog/sheet/alert-dialog 3 files)
    - Q5 γ Hybrid Phase A user-facing manual + Cursor self-verify internal
    - Q6 β Atomic per concern (foundation FIRST + consumer per bucket)
    - + Task 1.5 corrected re-audit (regex bug fix: `accent-` prefix false positive ~37 occ / 13 files)
    - + Task 8 X-Ray 0-net-migration discovery (10/10 keep-raw all precedents apply)
    - + 5 SWOT sub-bucket pre-decision locks (cross 9a/9b/9c consistent semantic decision lock prevent ambiguous round-trips — Pattern L58 NEW)

  - **Smoke test status — deferred manual Phase A anh Vũ Hải khi tiện**:

    | Priority | Route | Bucket | Pattern |
    |---|---|---|---|
    | HIGH | `/lien-he` | lien-he | 5 form inputs `bg-card` warm card |
    | HIGH | `/login` + `/register` + `/update-password` + `/reset-password` | Auth | Red error inline + Google btn elevated + dark panel preserved |
    ...
    | MEDIUM | `/dashboard/settings` + `/dashboard/discovery` (+ swot/pain-mapper/vision-workshop/synthesis/benchmark/xray-history) | Dashboard 11a + 11c | Form inputs + gradient stickers + CTA brand red |
    | MEDIUM | `/dashboard/x-matrix/new` | X-Matrix canvas | CenterX KPI badge `kpi-attention-strong` + HoshinEditModal |
    | MEDIUM | `/dashboard/discovery/swot` (workshop + canvas + synthesis) | SWOT 9a/9b/9c | TowsCanvas + SwotContextForm + TowsStrategy + SwotFinalizeList |
    | LOW | `/admin/hoshin-explorer` | Admin (super-admin) | 2-color category keep-raw preserved |

    Phase B Cursor self-verify implicit qua typecheck + build PASS cross 14 atomic commits (L42 partial coverage reinforced lần 8).

  - **Pattern lessons** (2 mới L58 + L59, 3 reinforced L42 + L48 + L50):
    1. **L58 NEW — Pre-decision lock pattern cho large bucket milestone save ambiguous round-trips**: 7 Q-cases pre-locked cross 3 SWOT sub-buckets (9a/9b/9c) saved ~10 round-trip ambiguous prompts. Pattern: BEFORE start sub-bucket execution, enumerate ambiguous decisions (semantic surface ambiguity, keep-raw vs migrate boundary, gradient handling, dynamic hex from data) + resolve cross-bucket consistent. Apply universally future large-scope cleanup milestones (≥5 sub-buckets). Cost ~10-15 phút decision lock, save ~30-60 phút avoid round-trip mid-execution.
    2. **L59 NEW — Baseline grep count ≠ migration count, real ratio ~60-65% cumulative avg**: Task 1.5 corrected re-audit (306 raw + 32 arbitrary hex = 338 baseline) → actual migrations ~200 (65% ratio). Per-occurrence semantic context audit (NOT bulk grep replace) reveal keep-raw justified majority cases. Apply universally future raw palette / token migration milestones: budget execution time at 60-70% baseline grep count, not 100%.
    3. **L42 reinforced lần 8** — Phase A visual partial coverage acceptable cho design refactor mechanical 1:1 no business logic. Pattern proven 8 lần cumulative.
    4. **L48 reinforced lần 11** — Foundation completion check pre consumer migration. 2 foundation commits ship FIRST trước 9 consumer buckets.
    5. **L50 reinforced lần 6** — Atomic commit boundary discipline per concern. Dashboard 11a/11b/11c + SWOT 9a/9b/9c sub-split proven scalable cho large bucket diff.
...
- **Latest milestone**: M-Design-Dark-1.5 hotfix (Soft hide dark mode UI, force light permanent — 1 commit `67919e0`, ~30 phút work). Trigger: Vũ Hải visual feedback aesthetic concern Q5 α dark mute pastel "trùng màu không nhìn rõ" post-ship M-Design-Dark-1 production verify. Decision Route A Soft hide UI: revert 3 changes (layout.tsx:114 inline script force light + layout.tsx:154 `enableSystem={false}` + header.tsx remove DropdownMenu Sun/Moon/Monitor toggle entirely + cleanup imports useTheme/Sun/Moon/Monitor) + KEEP CSS infrastructure (`.dark` block ~80 LOC dead CSS + `theme-cache-bridge.tsx` dead code) + foundation REFERENCE form (Commit 2 LITERAL→REFERENCE architectural change preserved). Future re-enable: 1 commit toggle inline script + `enableSystem={true}` + re-add DropdownMenu UI. M-Design-Dark-1 status: SHIPPED → ⚠️ HIDDEN UI (infrastructure preserved cho future re-enable).

  - **Files changed (2 MODIFIED + 0 DELETE — KEEP infrastructure)**:
    - MODIFIED: [app/layout.tsx](app/layout.tsx) (line 114 inline script force light + line 154 `enableSystem={false}` revert)
    - MODIFIED: [components/layout/header.tsx](components/layout/header.tsx) (DropdownMenu Sun/Moon/Monitor toggle removed entirely lines 96-139 → comment-only marker, cleanup imports `useTheme` from next-themes + `Sun/Moon/Monitor` from lucide-react)
    - PRESERVED: [components/providers/theme-cache-bridge.tsx](components/providers/theme-cache-bridge.tsx) (dead code, useEffect listen `useTheme().resolvedTheme` never fire light-only mode)
  ...
  - **Constraints cho future AI sessions**:
    ...
    - KHÔNG remove `app/globals.css` `.dark` block ~80 LOC (preserve 5-layer cascade architecture L52 cho future re-enable).
    - KHÔNG re-introduce inline script 3-option support without Vũ Hải explicit approval (Q-aesthetic-route Bỏ luôn locked 2026-05-11).
    - KHÔNG re-import `Sun/Moon/Monitor` lucide icons HOẶC `useTheme` từ next-themes vào header.tsx without re-enabling toggle UI flow.
    - KHÔNG modify inline script `app/layout.tsx:114` force light snippet — regression guard cho dark unreachable. Anti-pattern: revert sang 3-option support → user thấy DropdownMenu missing → confusing UX.
    - KHI bring back dark mode future, propose visual sample TRƯỚC apply commit (Pattern L56 aesthetic verify post-ship trade-off).
    - KHI add UI feature subjective aesthetic-heavy (color palette, typography hierarchy, spacing rhythm), MUST propose visual sample (HTML mockup hoặc Figma reference) TRƯỚC apply commit. Pattern L56 reinforce.

- **Previous milestone**: M-Design-Dark-1 (Dark mode foundation full ship — brand + saturated + Pastel cascade + toggle UI 3-option + chart cache bridge, 7 commits `bbdca2d` plan → `5ed573a` refactor → `22cd969` brand foundation → `6ba6e03` saturated + border → `293e6b0` Pastel mute → `d29fc03` toggle UI + cache bridge → `74b1c65` close-out, ~5h45min cumulative work). Trigger: M-Design-3a/3b/Tokens-Cleanup-1 foundation shipped (KPI 8 + Score 4 + Pastel 6 + brand tokens + composite borders) NHƯNG `.dark` block chỉ override shadcn neutral + chart + sidebar (~30 tokens). Toàn bộ NB v3.2 custom (~30+ tokens) thiếu dark variants — brand foundation gap CATASTROPHIC (`--ink #1A1A1A` trên `--bg #1A1A1A` = text invisible). Path γ FULL locked Q1 (all tokens + toggle UI fix 1 milestone) + Q2 invert lab + Q4 β `--brand #E84947` align shadcn `--primary` + Q5 α dark mute pastel + Q6 γ refactor LITERAL→REFERENCE + Q7 α useEffect listen `useTheme().resolvedTheme` + Q8 α 7 commits no split. Total milestone delta ~+386 net LOC within Q1 γ FULL ~6-8h budget.

  - **Tasks shipped (7 commits)**:
    1. Commit 1 plan `bbdca2d` (343 LOC plan doc Task 1 + V1-V5 audit + Q4-Q8 decisions)
    2. Commit 2 refactor `5ed573a` (38 LITERAL→REFERENCE `@theme inline`, Group 1 NB colors 15 + Group 2 KPI 10 + Group 3 shadows 13)
    3. Commit 3 brand foundation `22cd969` (10 brand tokens `.dark` — `--ink/--bg/--bg-dark` hard flip + `--brand #E84947` brighten + `--text-2 #B5B3B0` + `--text-3 #8A8787` hierarchy parallel + `--bg-muted/-paper #3a3939` + `--white` stay `#FFFFFF` + `--brand-dark #C73937`)
    4. Commit 4 saturated + border `6ba6e03` (4 Score cascade `var(--brand)` + `#F59E0B` + `#3B82F6` + `#22C55E` saturated brighten + 2 KPI-strong `#22C55E` + `#F59E0B` + 1 `--border-heavy` LITERAL→REFERENCE Path R1)
    5. Commit 5 Pastel mute `293e6b0` (6 Pastel dark mute Q5 α: warm yellow `#4A4530` + teal `#2E4644` + olive `#3D4530` + mauve `#4A3839` + brown `#4A3D2E` + purple `#3D364A`)
    6. Commit 6 toggle UI + cache bridge `d29fc03` (Phase 6A inline script 3-option light/dark/system honor storage + matchMedia detect + Phase 6B `enableSystem={true}` + Phase 6C DropdownMenu Sun/Moon/Monitor Vietnamese labels Sáng/Tối/Hệ thống + Phase 6D NEW `theme-cache-bridge.tsx` wire ThemeProvider tree)
    7. Commit 7 close-out `74b1c65` (Smoke 8 cases Phase A + HANDOFF §16/§17/§18 + plan doc Section 5/6 archive)

  - **3 architectural changes**:
    1. **5-layer cascade architecture proven** (pattern L48 lần 18 reinforced cumulative). Layer 1: `@theme inline` REFERENCE form (Commit 2). Layer 2: `:root .dark` brand foundation override (Commit 3). Layer 3: Composite shorthand REFERENCE (Commit 4 `--border-heavy`). Layer 4: Saturated + Pastel `.dark` override (Commit 4-5). Layer 5: Toggle UI 3-option + chart cache bridge (Commit 6). Reusable cho future design system extensions multi-layer reference.
    2. **2-hop cascade pattern proven end-to-end** (Pastel → KPI base → consumer Tailwind class). KPI base tokens ALL REFERENCE form cascade qua `--accent-*` + `--brand` + `--ink` + `--white` → KHÔNG cần direct `.dark` override (verify-first audit invalidate plan claim "14 KPI direct override" → actual 0 needed). Reduce scope 14→0 tokens KPI direct override.
    3. **chart-tokens cache invalidation wire** — `clearTokenCache()` export tồn tại pre-milestone (dead code from M-Design-3a) → Commit 6 Phase 6D ThemeCacheBridge wire `useEffect` listen `useTheme().resolvedTheme` → fire `clearTokenCache()` side effect. Recharts dots + `resolveToken()` consumers fresh hex post toggle (KHÔNG cần page reload).

  - **8 decisions locked Task 1** (plans/_archive/shipped-2026-05-11/M-Design-Dark-1-plan.md):
    - Q1 γ FULL all tokens + toggle UI fix 1 milestone
    - Q2 Invert lab `--ink/--bg/--bg-dark` hard flip
    - Q3 Full 8 cases Phase A smoke
    - Q4 β `--brand #E84947` align shadcn `--primary` AA-normal pass
    - Q5 α Dark mute pastel preserve hue drop lightness
    - Q6 γ Refactor LITERAL→REFERENCE shadows eliminate dual-mirror
    - Q7 α useEffect listen `useTheme().resolvedTheme` clearTokenCache
    - Q8 α 7 commits 1 session no split
    + 5 mid-milestone Q-decision lock (Q-shadow Accept 3px mobile + Q-push-timing batch cuối milestone + Q-c6-visual-verify PASS 4 paths + Q-smoke-scope Full 8 cases + Q-toggle-scope 3-option full)

  - **Smoke test 8/8 Phase A PASS pending Vũ Hải verify manual 2026-05-11 post-push** (template — anh fill verdict per case post-push verify chain):

    | Case | Route | Component | Light | Dark | Pattern |
    |---|---|---|---|---|---|
    | 1 | /dashboard/kpi | KpiCard 3-tier | bg-kpi-{healthy/attention/warning} pastel + saturated text | dark mute cascade (lime→olive #3D4530, amber→warm yellow #4A4530, pink→mauve #4A3839) + saturated text brighten | Pastel cascade 2-hop |
    | 2 | /dashboard/kpi | KpiSparkline | strokes #16A34A + #D97706 + #ef4444 light | strokes brighten dark #22C55E + #F59E0B + #ef4444 | KPI-strong saturated |
    | 3 | /dashboard/discovery/xray-history | XRayHistoryChart Recharts | tier dots light hex | tier dots brighten dark + cache invalidate via ThemeCacheBridge | Score 4-tier + chart cache |
    | 4 | /dashboard/settings | Generic card surface | bg-bg-paper #F5F0E8 light | bg-bg-paper #3a3939 dark mute | Brand foundation cascade |
    | 5 | / Sidebar | Sidebar bg + brand red active | shadcn sidebar light tokens | shadcn sidebar dark + brand red active --brand #E84947 | shadcn integration |
    | 6 | / Header | DropdownMenu 3-option Sun/Moon/Monitor + Vietnamese labels | Sáng/Tối/Hệ thống + Sun icon active | Moon icon active + click "Hệ thống" → resolvedTheme match OS preference | Toggle UI 3-option |
    | 7 | /dashboard/x-matrix/new | GembaModal hoặc HoshinEditModal | modal bg + border + text light | modal bg dark + --border-heavy cascade #FFFEF9 visible | Composite border refactor |
    | 8 | / Hero (HeroNB) | 3 stickers VISION/GOALS/KPIs pastel | bg-accent-{yellow/cyan/pink} light pastel | dark mute cascade (warm yellow + teal + mauve) | Pastel cascade end-to-end |

    Bonus check end-to-end: (a) Toggle "Hệ thống" → DevTools simulate prefers-color-scheme dark → app theme tự cập nhật real-time (`enableSystem={true}` listen mediaQuery), (b) Hard reload page với theme 'dark' → KHÔNG flash light (SSR flash prevention inline script Phase 6A), (c) Toggle Light↔Dark NGAY mà KHÔNG reload → Recharts dot color update fresh hex (ThemeCacheBridge useEffect).

  - **Files changed (1 NEW + 4 MODIFIED + 1 plan doc archived)**:
    - NEW: [components/providers/theme-cache-bridge.tsx](components/providers/theme-cache-bridge.tsx) (~27 LOC, ThemeCacheBridge useEffect listen `useTheme().resolvedTheme` → `clearTokenCache()` side effect)
    - NEW: `plans/_archive/shipped-2026-05-11/M-Design-Dark-1-plan.md` (343 LOC, Task 1 audit V1-V5 + Q4-Q8 + Section 5/6 close-out)
    - MODIFIED: [app/globals.css](app/globals.css) cumulative: Commit 2 LITERAL→REFERENCE 38 tokens + Commit 3 brand foundation 10 tokens `.dark` + Commit 4 saturated + border 7 tokens delta + Commit 5 Pastel mute 6 tokens
    - MODIFIED: [app/layout.tsx](app/layout.tsx) (Phase 6A inline script 3-option light/dark/system honor storage + matchMedia detect)
    - MODIFIED: [components/providers/theme-provider.tsx](components/providers/theme-provider.tsx) (Phase 6B `enableSystem={true}` + ThemeCacheBridge wire INSIDE NextThemesProvider)
    - MODIFIED: [components/layout/header.tsx](components/layout/header.tsx) (Phase 6C DropdownMenu 3-option Sun/Moon/Monitor + Vietnamese labels Sáng/Tối/Hệ thống)

  - **Pattern lessons** (3 mới L52-L54):
    1. **L52 — 5-layer cascade architecture cho design system multi-layer reference**: Foundation refactor LITERAL→REFERENCE eliminate dual-mirror pattern L48 reinforced lần 18. 5 layers proven (`@theme` + `:root` brand + composite refactor + saturated/Pastel override + toggle bridge). Apply universally future design system extensions cần dark mode hoặc theme variants. KHÔNG refactor riêng từng layer cứng đầu — bundle cumulative cascade architecture.
    2. **L53 — Verify-first audit ROI proven 4 milestones cumulative** (Tokens-Cleanup-1 + Tailwind-Cleanup-1 + RateLimit-Generic-1 + Dark-1). Pattern: pre-Commit audit-first reduce scope/catch drift consistent. Cost ~15-30 phút audit, save ~30-90 phút round-trip drift fix per cumulative Commit. Apply DEFAULT cho future design refactor + cross-cutting helper milestones, KHÔNG trust HANDOFF prose + plan doc claim blindly.
    3. **L54 — Idempotent guard pattern Edit tool**: Khi prompt yêu cầu apply commit/edit nhưng verify-first phát hiện current state ALREADY post-apply (file content match expected post-state), MUST: (a) SKIP re-apply (Edit tool sẽ FAIL match old_string), (b) Confirm git log + working tree clean, (c) Re-verify typecheck + build baseline preserved, (d) Report idempotent state thay vì attempt no-op. Apply universally — TRƯỚC Edit tool, verify file content match expected pre-state. Anti-pattern: blind re-apply → Edit fail + waste tool calls.

  - **Constraints cho future AI sessions**:
    - KHÔNG re-add inline script `app/layout.tsx:113` force light snippet (Phase 6A 3-option support locked).
    - KHÔNG modify `enableSystem={true}` (Phase 6B locked) — disable system preference detect = breaking change UX modern OS-respect convention.
    - KHÔNG remove `ThemeCacheBridge` component (Phase 6D locked) — Recharts cache invalidation depend on this. Anti-pattern: bypass bridge → cache stale post toggle → Recharts dots render wrong hex.
    - KHÔNG modify default theme value `'light'` (preserve brand identity light-first). User explicit choose 'system' để OS-respect.
    - KHÔNG modify `--white` token `.dark` stay `#FFFFFF` (semantic preserve "high-contrast text on saturated/brand surface"). Anti-pattern flip black: `--kpi-critical-fg` consumer break (black text trên `--brand-dark #E84947` = LOW CONTRAST FAIL).
    - KHÔNG re-introduce LITERAL hex pattern `:root` block cho composite shorthand tokens (`--border-*`, `--shadow-*`) — Path R1 REFERENCE form mandatory cho dark mode cascade. Pattern L48 lần 18 reinforced.
    - KHÔNG remove `enableSystem={true}` mà KHÔNG đồng thời remove Monitor icon DropdownMenuItem header.tsx (consistency UI ↔ provider config).
    - KHI add token mới có dark variants needed, follow 5-layer pattern: (1) `:root` light value, (2) `:root .dark` override hex, (3) `@theme inline --color-{name}: var(--{name})` REFERENCE, (4) verify Tailwind class consumer auto-cascade, (5) skip direct `.dark` override nếu token là REFERENCE form aliasing existing foundation.
    - KHI add Recharts custom component consume `resolveToken()`, MUST hoist module-level + props pass (L46 lint rule static-components) — ThemeCacheBridge KHÔNG affect AST inspection.
    - KHÔNG re-bundle Pastel `.dark` override với KPI base direct override mid-future milestone — verify-first audit-first invalidate "KPI direct override needed" claim. KPI base ALL REFERENCE form auto-cascade qua Pastel + brand + ink + white.

- **Previous milestone**: M-Design-Tokens-Cleanup-1 (`--accent` collision cleanup + `.heading-overline` consolidation, 4 commits `eb34541` foundation → `20ed62f` XRayReport → `2c6f976` consolidate + `a389632` close-out, 26 files: 1 NEW + 25 MODIFIED, ~1.5h work). Trigger: M-Design-3a foundation discovered `app/globals.css` line 171 reassign `--accent: #c73937` override shadcn neutral pastel `#F5F0E8` (line 136), contradict explicit comment line 158 cảnh báo collision. Vũ Hải screenshot 2026-05-10 evidence visible regression — KPI dropdown 3-dots hover + org switcher dropdown flash brand red rực gây eye-fatigue. Path β REVERT shadcn default lock với evidence distribution Nhóm A:B:C = 7:17:0 (intentional brand:accidental shadcn:ambiguous) + layout team đã pioneer pattern `--accent-brand` token (`@theme line 50` hardcoded `#c73937`, separate từ `--accent`) cho intentional brand-as-Tailwind-class.
  ...
  - **Pattern lessons (2 mới L50 + L51, 2 reinforced)**:
...
### 2026-05-11 — M-Design-Tangential-Cleanup-1 — Raw Tailwind palette → NB v3.2 tokens cleanup

**Milestone**: M-Design-Tangential-Cleanup-1 — Raw Tailwind palette consumer migration sang NB v3.2 design tokens, foundation completion check, atomic per concern sub-bucket discipline.

**Scope**: 14 atomic commits cross 9 consumer buckets + 2 foundation CSS commits + 1 audit-fix doc-only commit + 1 close-out (this chain). ~65 .tsx files MODIFIED + `app/globals.css` 2 foundation edits + plan doc archive. ~6h Cursor execution + close-out. Risk LOW build / MEDIUM visual user-facing (Phase A smoke deferred manual). Driver: M-Design-Tailwind-Cleanup-1 V3 audit defer trigger (~28 tangential files estimate, real ~65) + M-Design-Dark-1.5 force-light lock context (no dark variants needed) + M-Design-Tokens-Cleanup-1 §17 `--white` preservation constraint precedent.

**Methodology source**: M-Design-Tailwind-Cleanup-1 Q1 β (directory-bucket atomic commit) + Q2 α (file-by-file scope within bucket) + L48 foundation completion check pre consumer migration. M-Design-Tokens-Cleanup-1 L50 atomic commit boundary discipline per concern. Verify-first L29/L32/L45/L48/L49 reinforced lần 9 — Task 1.5 corrected re-audit Bug surfaced Commit 3 (`accent-` regex false positive ~37 occ / 13 files inflation), per-occurrence semantic audit reveal keep-raw justified majority.

**3-4 Architectural decisions locked**:

1. **Q1 β commit cadence directory-bucket atomic scalable cross 14 commits + 9 sub-buckets (Pattern L50 reinforced lần 6)**: Dashboard 11a/11b/11c sub-split (surface vs KPI status vs discovery decoration) + SWOT 9a/9b/9c sub-split (Workshop+Chat vs Context+Drafts+Canvas vs TOWS+Synthesis+Finalize) scale gracefully cho large diff bucket (>200 LOC). Each sub-commit ~30-60 min effort, reversibility granular per concern. Anti-pattern rejected: single bucket commit >500 LOC diff (lose atomic value, hard to bisect visual regression). Apply universally future ≥5-sub-bucket cleanup milestones.

2. **Q2 default mapping table + Q2-sub-α extended cross 5 bucket precedent**: Keep-raw scope established uniform cross consumer buckets — (a) Q2-sub-α white-alpha on dark surface (Layout sidebar + Page footer + Auth dark panel + X-Ray CTA banner), (b) Q2 extended white text on saturated-brand surface (Layout header button + Auth Google btn + X-Ray pills/checkmark + Annual-review status pills), (c) Ambig 1 C elevated `bg-white` cards on warm cream (Auth Google btn + X-Ray option pill + SWOT framework card), (d) Hoshin Explorer 2-color category data-driven hex keep-raw (admin super-admin only), (e) AI sparkle decorative hex keep-raw (one-off non-themable). Anti-pattern rejected: bulk regex replace ignoring semantic context → break sidebar text contrast + button readability + X-Ray CTA banner. Pattern L59 NEW emerged (baseline grep count ≠ migration count, ratio ~65% cumulative).

3. **Q6 β atomic per concern proven L48 + L50 cumulative**: 2 foundation commits ship FIRST trước 9 consumer buckets — `.card-subtle` (Commit 1 `b0edc91`) + `.input-brutal` (Commit 4 `0d6a792`) both `var(--white)` → `var(--card)` to align elevated card semantics with NB v3.2 warm cream. `:root` `--card` token reuse validated (existing shadcn token, NO net-new introduction). Tailwind v4 `@theme inline --color-card` mirror exists pre-milestone — L48 foundation parity preserved without milestone delta. Constraint: foundation token consume order MUST verify-first (audit checklist L48) — `:root` defined + `@theme inline` mirror BOTH required cho Tailwind v4 class emit.

4. **Pre-decision lock pattern for large bucket milestone (L58 NEW)**: 7 Q-cases pre-locked cross 3 SWOT sub-buckets (9a/9b/9c) saved ~10 round-trip ambiguous prompts. Pattern: BEFORE start sub-bucket execution, enumerate ambiguous decisions (semantic surface ambiguity, keep-raw vs migrate boundary, gradient handling, dynamic hex from data) + resolve cross-bucket consistent. Cost ~10-15 phút decision lock, save ~30-60 phút avoid round-trip mid-execution.

**Constraints cho future AI sessions**:

- KHÔNG modify foundation tokens (`--card`, `--white`, `--bg`, `--bg-warm`, `--bg-muted`, `--bg-paper`, `--bg-dark`, `--ink`, `--text-2`, `--text-3`, `--brand`, `--accent-*`, `--kpi-*` family) without explicit decision lock — semantic role established cumulative cross M-Design-3a/3b/Tokens-Cleanup-1/Tailwind-Cleanup-1/Dark-1/Tangential-Cleanup-1.
- KHÔNG re-introduce raw `bg-white` / `bg-gray-*` / `text-gray-*` Tailwind palette utilities cho new code — pattern `bg-card` / `bg-bg-warm` / `text-text-2` token-based class. Regression guard: cleanup milestone scope locked 2026-05-11.
- KHÔNG migrate keep-raw white-on-saturated-surface intentional contrast cases (Q2-sub-α + Q2 extended). Anti-pattern: bulk regex replace → break sidebar text contrast + button readability + X-Ray CTA banner. Cross 5-bucket precedent established (Layout/Page footer + Auth dark panel + X-Ray + Hoshin Explorer + AI sparkle).
- KHÔNG add `dark:*` variant cho new code — defer M-Design-Dark-1 re-enable milestone nếu user complain (M-Design-Dark-1.5 force-light lock 2026-05-11). 0 `dark:*` variants in repo cross-milestone.
- KHÔNG invent new `--bg-X` foundation token without verify-first confirm gap (L48 audit checklist). Pattern adopt-over-create reinforce — fragmentation cost > clean naming benefit.
- KHÔNG bundle DRY follow-ups (N1 lien-he 5 inline form inputs → `.input-brutal` className) vào close-out — defer M-Design-DRY-Cleanup-1 candidate riêng. Anti-pattern: scope creep cuối milestone → blur atomic boundary.
- KHI cleanup milestone large-scope (≥5 sub-buckets), apply L58 pre-decision lock pattern: enumerate ambiguous Q-cases cross sub-buckets + resolve consistent BEFORE start execution.
- KHI estimate raw palette migration effort, budget 60-70% baseline grep count (L59 pattern) — keep-raw justified majority cases via per-occurrence semantic audit.

**Pattern lessons (2 mới L58 + L59, 3 reinforced L42 + L48 + L50)**: xem §16 entry detail.

---

### 2026-05-11 — M-Design-Dark-1.5 hotfix Soft hide UI (post Dark-1 ship)

**Trigger**: Aesthetic feedback Q5 α dark mute pastel "trùng màu không nhìn rõ" post-ship M-Design-Dark-1 production verify (cùng ngày). Decision Route A Soft hide UI thay vì Q5 γ Hybrid hotfix hoặc hard revert toàn bộ Dark-1 work.

**Scope**: 1 commit `67919e0`, 2 files MODIFIED (`app/layout.tsx` + `components/layout/header.tsx`), KEEP infrastructure (`theme-cache-bridge.tsx` + `globals.css .dark` block + foundation REFERENCE form Commit 2). ~30 phút work.

**Architectural decisions locked**:

1. **Soft hide UI over hard revert**: Preserve 5-layer cascade architecture (Pattern L52) + LITERAL→REFERENCE `@theme inline` foundation (Commit 2) + ThemeCacheBridge wire pattern (Phase 6D). Future re-enable cost ~1 commit toggle inline script + `enableSystem={true}` + re-add DropdownMenu UI. Anti-pattern rejected: hard revert all Dark-1 work — would lose 5h45min infrastructure investment + force re-architect future dark mode attempt.
...
### 2026-05-11 — Dark mode foundation full ship (M-Design-Dark-1)

**Milestone**: M-Design-Dark-1 — Dark mode foundation cumulative cascade architecture (brand + saturated + Pastel + toggle UI + chart cache).

**Scope**: 7 commits `bbdca2d`→`74b1c65`, 1 NEW + 4 MODIFIED + 1 plan doc archived, ~+386 net LOC, ~5h45min cumulative work. Risk MEDIUM-HIGH (Q1 γ FULL plan flag — UI component scope creep + edge cases SSR flash + system preference detect) → ahead schedule nhờ verify-first audit reduce scope 50%+ pre-Commit 4 (KPI base direct override 14→0 tokens). Driver: M-Design-3a/3b/Tokens-Cleanup-1 foundation shipped 5-10 ngày trước NHƯNG `.dark` block thiếu ~30+ NB v3.2 custom tokens — brand foundation gap CATASTROPHIC (text/border/shadow invisible trên dark bg).

**Methodology source**: 5-layer cascade architecture leverage existing foundation (M-Design-3a chart-tokens.ts resolver + Tokens-Cleanup-1 `--accent-brand` token riêng + Tailwind-Cleanup-1 KPI/discovery hub consumer migrate). Verify-first audit-first cost ~15-30 phút reduce scope 50%+ proven 4 milestones cumulative (L53 NEW pattern lesson).

**3 Architectural decisions locked**:

1. **5-layer cascade architecture pattern (L52 NEW)**: Foundation refactor LITERAL→REFERENCE `@theme inline` + `:root` brand foundation `.dark` + composite shorthand REFERENCE + saturated/Pastel `.dark` override + toggle UI 3-option + chart cache bridge. 5 layers proven cumulative cascade architecture. Apply universally future design system extensions cần dark mode hoặc theme variants. Constraint: KHÔNG refactor riêng từng layer cứng đầu — bundle cumulative cascade architecture.

2. **2-hop cascade pattern proven end-to-end**: KPI base tokens ALL REFERENCE form (`var(--accent-lime)`, `var(--brand)`, `var(--ink)`, `var(--white)`) cascade qua foundation Commit 3 + Pastel `.dark` override Commit 5 → consumer Tailwind class `bg-kpi-healthy` resolve runtime cascade chain hoạt động end-to-end. KHÔNG cần direct `.dark` override KPI base tokens (verify-first invalidate plan claim "14 KPI direct override needed" → actual 0). Reduces scope 14→0 tokens. Pattern L48 lần 18 reinforced.

3. **Idempotent guard Edit tool pattern (L54 NEW)**: TRƯỚC Edit tool apply commit/edit, MUST verify file content match expected pre-state. Nếu current state ALREADY post-apply → SKIP re-apply + report idempotent state. Discovered Phase 6E Commit 6 — Cursor verify-first detect `d29fc03` đã ship trước → SKIP no-op re-apply Phase 6A-6D. Apply universally Edit tool calls future. Anti-pattern: blind re-apply → Edit fail + waste tool calls.

**Constraints cho future AI sessions** (xem §16 entry full list).

**Pattern lessons** (3 mới L52-L54): xem §16 entry detail.
...
- **L51 NEW — Log noise vs user-facing crash discrimination**: Vercel runtime logs có thể chứa entries với severity = error NHƯNG status code = 307/200 (redirect/success). Đây là render error sau redirect pipeline, severity thực tế THẤP — user thật KHÔNG bị ảnh hưởng. Khác với severity = error + status = 500 (user-facing crash, MUST fix immediately). M-Design-Tokens-Cleanup-1 verify chain phát hiện pattern này: TypeError /dashboard 307 trigger bởi curl unauthenticated → page component access user data trước middleware redirect kick in. Status 307 redirect đúng → user-facing impact = 0, chỉ log noise. **Audit checklist khi gặp Vercel runtime error log**: (a) Status code trả về user là gì? (307/200 = log noise; 500/timeout = user-facing crash) — (b) Reproduce trigger source: user thật vs bot/curl/Playwright? (curl unauth = artifact testing, không reflect user reality) — (c) Logic scope milestone gần đây có thể impact route đó không? (impossibility argument từ scope = strong evidence non-regression) — (d) Pre-existing verify: deploy cũ có cùng pattern không? (CAVEAT L41 — Vercel log expire deploy cũ, INCONCLUSIVE possible). **Action mapping**: Status user-facing OK + reproduce non-user trigger → DEFER LOW (log candidate milestone investigate); Status user-facing OK + reproduce user trigger → DEFER MEDIUM (investigate within next 1-2 milestones); Status user-facing CRASH (500/timeout) → BLOCK ship + investigate immediately. Apply universally cho future Vercel verify chain runtime logs review. Apply Vercel inspector deep-dive khi cần full stack trace (Vercel MCP truncate).
...
- **M-Design-Tangential-Cleanup-1 — Raw Tailwind palette → NB v3.2 tokens cleanup** ✅ SHIPPED 2026-05-11 (14 atomic commits across 9 consumer buckets + 2 foundation + 1 audit-fix + 1 close-out chain, ~65 .tsx files + `app/globals.css` 2 foundation edits + plan doc archive, ~200 real migrations / ~70 keep-raw documented / 65% migration ratio, ~6h Cursor execution). Trigger: M-Design-Tailwind-Cleanup-1 V3 audit defer (~28 tangential files estimate, real ~65). Foundation Commit 1 `b0edc91` (.card-subtle `var(--white)` → `var(--card)`) + Commit 4 `0d6a792` (.input-brutal parallel) + Task 1.5 corrected re-audit `a922ed3` (regex bug fix `accent-` prefix false positive ~37 occ / 13 files inflation) + 9 consumer buckets: lien-he `0d220cc` + X-Matrix canvas `947a2c4` + Auth `b6846a6` + X-Ray doc-only `955036a` + Annual-review `ce17b7d` + Admin `29fe6f5` + Dashboard 11a/11b/11c `3a3a883`/`cb4a365`/`0825f2c` + SWOT 9a/9b/9c `c8a8e01`/`276307c`/`4a8de48`. 7 Q-decisions locked (Q1 β commit cadence + Q2 default mapping + Q2-sub-α extended cross 5 bucket precedent + Q4 α skip shadcn + Q5 γ hybrid smoke + Q6 β atomic per concern + Q7 effort) + Task 1.5 audit fix + Task 8 X-Ray 0-net-migration discovery + 5 SWOT sub-bucket pre-decision locks. 0 net-new foundation tokens (L48 verified existing token family consumes). 0 business logic / state / JSX structure delta. Phase A user-facing smoke deferred manual anh Vũ Hải khi tiện cross 10 routes. 2 NEW pattern lessons: **L58 (pre-decision lock pattern cho large bucket milestone save ~10 round-trips)** + **L59 (baseline grep count ≠ migration count, real ratio ~60-65% cumulative — keep-raw justified majority via per-occurrence semantic audit)**. 3 reinforced: L42 lần 8 + L48 lần 11 + L50 lần 6. See §16 + §17 detail. Plan doc archived: `plans/_archive/shipped-2026-05-11/M-Design-Tangential-Cleanup-1-plan.md`.
...
1. **M-Design-DRY-Cleanup-1** (NEW from M-Design-Tangential-Cleanup-1 N1 deferred, LOW priority): `app/lien-he/page.tsx` lines 141-201 inline-duplicates `.input-brutal` design system pattern (5× input/select/textarea elements). Action: Replace 5 inline className blocks với `className="input-brutal"`. Now that `.input-brutal` foundation Commit 4 `0d6a792` uses `var(--card)` (matching lien-he Commit 2 `0d220cc` `bg-card` migration), visual will be identical post-refactor. Net: removes ~30 LOC design system duplication. Cost ~10 phút. Trigger: bundle next design milestone HOẶC standalone follow-up commit khi tiện. Currently DEFER LOW.
2. **M-Design-Dark-1 re-enable** (NEW from M-Design-Dark-1.5 L57 hidden UI status, NICE-TO-HAVE): Re-enable dark mode toggle UI (~1 commit toggle inline script + `enableSystem={true}` + re-add DropdownMenu Sun/Moon/Monitor). Infrastructure preserved dormant (`.dark` block + `theme-cache-bridge.tsx` + foundation REFERENCE form). Re-enable cost ~30 phút vs fresh re-implementation ~5h45min. Trigger: user complain dark mode missing HOẶC design system extension need theme variants. Currently DEFER — Vũ Hải accept force-light permanent default 2026-05-11. Pattern L56 reinforce: propose visual sample (HTML mockup) TRƯỚC apply commit if re-engaging dark mute pastel decision.
...
- ~~**M-Design-Dark-1**~~ ✅ SHIPPED 2026-05-11 (7 commits `bbdca2d`→`74b1c65`, 5-layer cascade architecture). See "Shipped milestones (recent)" above + §16 + §17 architecture decision 2026-05-11.
...
- **M-Dashboard-NullGuard-1** (NEW from M-Design-Tokens-Cleanup-1 Vercel verify discovery, LOW priority): Investigate TypeError 'Cannot read prop...' trên `/dashboard` page render khi session=null trước middleware redirect kick in. Pattern Next.js App Router common khi page component access user/membership data BEFORE middleware refresh session cookies. **Severity LOW**: status 307 redirect đúng spec → user-facing impact = 0 (user thật vẫn được đẩy về /login). Chỉ là log noise của error boundary catch sau redirect. Reproduce deterministic mỗi curl unauthenticated → /dashboard. **Investigate plan**: (a) mở Vercel inspector https://vercel.com/vuhuyhais-projects/hoshin-kanri-os/<deploy_id> full stack trace identify file/property null, (b) likely culprit `app/dashboard/page.tsx` hoặc `app/dashboard/layout.tsx` access `user.something` hoặc `membership.org_id` trước auth guard, (c) add null guard `if (!user) redirect('/login')` BEFORE access user-derived data, hoặc move data fetch sau auth check. Cost ~30 phút investigate + 1 commit fix. **Trigger condition**: nếu phát hiện user-facing impact (vd error toast, render glitch), bump priority HIGH. Currently DEFER LOW — log noise only. Pattern lesson L51 §17 apply: log noise vs user-facing crash discrimination. Discovery evidence: Vercel runtime logs commit `a389632` 16:16:28 UTC + verify pre-existing INCONCLUSIVE (L41 deploy cũ log expired).
