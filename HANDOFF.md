...
## 16. Current State Snapshot (2026-05-09 — post M-KPI-Mgmt-1)
...
- **M-KPI-Restore-1 (Phase 2 deferred from M-KPI-Mgmt-1, LOW priority)**: Restore archived KPIs UI cho CEO. Endpoint POST `/api/kpi/[id]/restore` (toggle `is_active=true`), Settings/Admin page list archived KPIs với "Khôi phục" button. **Trigger conditions**: (1) user complain "lỡ tay xóa KPI", hoặc (2) audit trail show ≥3 archived KPIs unintended trong production. Cost ~1-2h. Currently DEFER (Q2 α + Q3 γ decision lock M-KPI-Mgmt-1 — Xóa MVP only). Constraint: KHI ship MUST add to /admin super-admin scope first (recovery dashboard pattern), KHÔNG phải dashboard CEO inline (avoid normalize destructive-then-undo workflow).
...
- **M-Lint-Cleanup-1 (NEW from M-Cleanup-5 Task 2 lint isolation, LOW priority)**: Fix 2 pre-existing lint errors verified via git stash isolation (M-Cleanup-5 KHÔNG introduce):
  - `react-hooks/static-components` (1 file — location TBD, grep cần)
  - `app/invite/[token]/page.tsx:75` `no-html-link-for-pages` (1-line fix: `<a href="/login">` → `<Link href="/login">` + import next/link)

  Cost ~30 phút total. Trigger: bất kỳ milestone touch `app/invite/*` hoặc khi want clean lint baseline cho strict CI gate. Pattern lesson L35 (verify isolation via git stash) applied trong M-Cleanup-5 Task 2.
