...
- **Last verified**: 2026-05-02 — post hotfix `df3c1ef` (AI Coach SWOT JSON parser 3-tier fallback chain, `max_tokens` bump 800→4096) on top of M-Design-3b (Dashboard hex-to-token refactor: 6 commits `868fa34`→`ed27932`, 4 files refactored, 0 raw hex còn trong logic). HEAD `df3c1ef`. Hotfix triggered by production user screenshot — `/api/swot/coaching` returned raw JSON blob `{"message":"...","extractedInsight":{...}}` + literal `\n\n` to ReactMarkdown when `JSON.parse` failed on truncated Vietnamese response. Foundation extended (`--score-{critical,weak,fair,good}` saturated 4-tier + `--kpi-{healthy,attention}-strong` saturated variants + `withAlpha()` helper) + 4 consumer refactors (xray-history page + chart, KpiSparkline, discovery hub badges + checkmarks). Preceded by M-Design-3a (KPI tokens foundation, 3 commits `d7fdb6d`→`b3ff123`).
...
- **M-AICoach-AutoFill-1** (NEW from hotfix `df3c1ef` diagnose, MEDIUM priority): Wire `extractedInsight` auto-fill SWOT card khi AI return valid structured insight. Discovered scope gap during JSON parser hotfix: server gửi `extractedInsight` về `CoachingResponse` ([app/api/swot/coaching/route.ts:138](app/api/swot/coaching/route.ts#L138)) NHƯNG client `SwotWorkshopChat` ([components/swot/SwotWorkshopChat.tsx:80-85](components/swot/SwotWorkshopChat.tsx#L80-L85)) KHÔNG consume — user phải manual click "Rút ý từ chat" để trigger `onAddIngredient`. Confusing UX. Scope: trên valid `extractedInsight`, auto-call `onAddIngredient(quadrant, statement)` + show toast "✓ Đã tự thêm vào [S/W/O/T]" với undo affordance. Effort ~50-100 LOC, 2-3 commits, 1 session. Trigger: user complain (đã 1 lần report screenshot 2026-05-02).
